...
import type { PaperlessResponse } from "@/app/api/widgets/paperless/types"

interface PaperlessWidgetProps {
  widgetId: string
...
function SettingsPanel({
  ...
  return (
    <div className="flex h-full w-full flex-col rounded-lg border border-border bg-card overflow-hidden">
      <WidgetHeader icon={File01Icon} title="Paperless-ngx" isSettings onSettingsClick={onClose} />
      <div className="flex-1 overflow-y-auto p-3 space-y-4">
        <div className="space-y-1.5">
          <Label htmlFor="paperless-url">Service URL</Label>
          <Input
            id="paperless-url"
            value={serviceUrl}
        ...
        <div className="space-y-1.5">
          <Label htmlFor="paperless-secret">Secret Name</Label>
          <Input
            id="paperless-secret"
            value={secretName}
...
export function PaperlessWidget({
  ...
  config,
  onDelete,
}: PaperlessWidgetProps): React.ReactElement {
  ...
  const [data, setData] = useState<PaperlessResponse | null>(null)
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/widgets/paperless?widgetId=${encodeURIComponent(widgetId)}`
      )
      ...
      const json = (await res.json()) as PaperlessResponse
      setError(null)
      setData(json)
    } catch {
        ...
        {loading ? (
          ...
          <>
            <div className="rounded-md bg-muted/50 px-2.5 py-2">
              <p className="text-[0.5625rem] text-muted-foreground">Total Documents</p>
              <p className="text-lg font-bold tabular-nums">{formatCount(data.totalDocuments)}</p>
            </div>
            ...
            <div className={cn(
              ...
              <p className={cn(
                ...
                data.inboxCount > 0 && "text-amber-500"
              )}>
                {formatCount(data.inboxCount)}
              </p>
            </div>

            <div className="grid grid-cols-3 gap-2">
              <StatCard label="Correspondents" value={formatCount(data.correspondents)} />
              <StatCard label="Tags" value={formatCount(data.tags)} />
              <StatCard label="Doc Types" value={formatCount(data.documentTypes)} />
            </div>
          </>
        ) : null}
      </div>
