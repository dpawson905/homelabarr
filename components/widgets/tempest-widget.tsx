...
import type { TempestResponse } from "@/app/api/widgets/tempest/types"

interface TempestWidgetProps {
  widgetId: string
...
export function TempestWidget({
  widgetId,
  config,
  onDelete,
}: TempestWidgetProps): React.ReactElement {
  ...
  const [data, setData] = useState<TempestResponse | null>(null)
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/widgets/tempest?${params}`)
      ...
      const json: TempestResponse = await res.json()
      setData(json)
      setError(null)
    } catch {
  ...
  const displayName = savedConfig.locationName || data?.station?.name || "Tempest"
    ...
    return (
      <div className="flex h-full w-full flex-col rounded-lg border border-border bg-card overflow-hidden">
        <WidgetHeader
          ...
          title="Tempest"
          isSettings
          settingsTitle={configured ? "Tempest Settings" : "Setup Tempest"}
          onSettingsClick={configured ? () => setShowSettings(false) : undefined}
        />

        <div className="flex-1 overflow-y-auto p-3 space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="tempest-station-id">Station ID</Label>
            <Input
          ...
          <div className="space-y-1.5">
            <Label htmlFor="tempest-secret">Secret Name</Label>
            <Input
              id="tempest-secret"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="tempest-location">Location Name (optional)</Label>
            <Input
              id="tempest-location"
